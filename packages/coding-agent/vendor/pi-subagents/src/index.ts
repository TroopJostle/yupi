...
import { getAgentConversation, getDefaultMaxTurns, getGraceTurns, getRememberAgents, normalizeMaxTurns, resolveEffectiveMaxTurns, SUBAGENT_TOOL_NAMES, setDefaultMaxTurns, setGraceTurns, setRememberAgents, steerAgent } from "./agent-runner.ts";
...
import { type AgentConfig, type AgentInvocation, type AgentMentionMode, type AgentRecord, type JoinMode, type NotificationDetails, type SubagentType, type ViewerMarkdownMode, type WidgetMode } from "./types.ts";
...
import { FleetList, type FleetUICtx, type FleetWorkflow } from "./ui/fleet-list.ts";
...
import { openWorkflowFromFleet, showWorkflowsMenu, type WorkflowMenuDeps } from "./ui/workflow-menu.ts";
...
import { fullWorkflowToolDescription } from "./workflow/tool-description.ts";
 ...
 *   tool (see test/fixtures/.yupi/agents/tools-none.md, which expects three). Calling
...
export default function (pi: ExtensionAPI) {
  ...
  let strictAgentFiles = loadSettings(configCwd).strictAgentFiles === true;

  /** Reload agents from project/global custom agent dirs and merge with defaults (called on init and each Agent invocation). */
  const reloadCustomAgents = (strict = false) => {
    const userAgents = loadCustomAgents(configCwd, strict);
  ...
  function startScheduler(ctx: ExtensionContext) {
    try {
      ...
      // extension keeps working if e.g. .yupi/ is unwritable.
...
Custom agents: .yupi/agents/<name>.md (project) or ${getAgentDir()}/agents/<name>.md (global).
...
Custom agents can be defined in .yupi/agents/<name>.md (project) or ${getAgentDir()}/agents/<name>.md (global) — they are picked up automatically. Project-level agents override global ones. Creating a .md file with the same name as a default agent overrides it.
...
**Never delegate understanding.** Don't write "based on your findings, fix the bug" or "based on the research, implement it." Those phrases push synthesis onto the agent instead of doing it yourself. Write prompts that prove you understood: include file paths, line numbers, what specifically to change.`;
  ...
  const loadCustomToolDescription = (): string | undefined => {
    for (const path of [
      join(process.cwd(), ".yupi", "agent-tool-description.md"),
  ...
  const agentTool = defineTool({
    ...
    parameters: Type.Object({
      ...
      subagent_type: Type.String({
        description: `The type of specialized agent to use. Available types: ${getAvailableTypes().join(", ")}. Custom agents from .yupi/agents/*.md (project) or ${getAgentDir()}/agents/*.md (global) are also available.`,
      }),
      model: Type.Optional(
        Type.String({
          description:
            'Optional model override. Use an exact "provider/modelId" from /model, or a fuzzy model name. Overrides the agent file\'s model. Omit to use the agent default; "inherit" explicitly uses the parent model. Unavailable models fail without substitution.',
        }),
      ),
    ...
    execute: async (toolCallId, params, signal, onUpdate, ctx) => {
      ...
      // Per-call model override, then agent default, then parent.
      let model = ctx.model;
      if (resolvedConfig.modelInput) {
        const resolved = resolveModel(resolvedConfig.modelInput, ctx.modelRegistry);
        if (typeof resolved === "string") {
          // Host contract: throw so the runtime marks the tool result as an error.
  ...
  const workflowTool = defineTool({
    ...
    parameters: Type.Object({
      ...
      name: Type.Optional(
        Type.String({
          description:
            "Name of a saved workflow — `<name>.js` in .yupi/workflows/, .agents/workflows/ or the user's agent dir. Lowest precedence: `scriptPath` and `script` both win over it.",
        }),
      ),
      args: Type.Optional(
        Type.Any({
    ...
    if (!isWorkflowsEnabled()) {
      report(
        `--${WORKFLOW_FILE_FLAG} ignored: workflows are off. Turn them on in /agents → Settings → Workflows, ` +
          'or set `"workflowsEnabled": true` in .yupi/subagents.json.',
        "warning",
      );
      return;
    }
  ...
  function getModelLabel(type: string, registry?: ModelRegistry): string {
    ...
    if (!cfg?.model || cfg.model === "inherit") return "inherit";
    ...
    function buildItems(): SettingItem[] {
      ...
      return [
        ...
        },
        {
          ...
          description: "Agent tool description sent to the LLM: full (rich, default), compact (~75% fewer tokens, for small/local models), or custom (.yupi/agent-tool-description.md with {{placeholders}})",
