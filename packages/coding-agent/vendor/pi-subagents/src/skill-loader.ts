 ...
 *   - <cwd>/.yupi/skills           (project, Pi's standard)
 *   - getAgentDir()/skills       (user, default ~/.yupi/agent/skills — Pi's standard)
 *
 * Layout per root:
...
function loadSkillContent(name: string, cwd: string): string {
  ...
  const roots = [
    join(cwd, ".yupi", "skills"), // project — Pi standard
  ...
  return `(Skill "${name}" not found in .yupi/skills/ or the Yupi user skills directory)`;
}
