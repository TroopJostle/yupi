...
import { createStructuredCapture, createStructuredOutputTool, structuredRetryPrompt } from "./structured-output.ts";
...
export function resolveDefaultModel(
  parentModel: Model<string> | undefined,
  registry: ModelRegistry,
  configModel?: string,
): Model<string> | undefined {
  if (!configModel || configModel.trim().toLowerCase() === "inherit") return parentModel;
  const model = resolveModel(configModel, registry);
...
export async function runAgent(
  ...
  const defaultSessionDir = process.env.YUPI_CODING_AGENT_SESSION_DIR ?? settingsManager.getSessionDir?.();
