 ...
 *
 * Memory scopes:
 *   - "user"    → getAgentDir()/agent-memory/{agent-name}/ (default ~/.yupi/agent/agent-memory/, honors $YUPI_CODING_AGENT_DIR)
 *   - "project" → .yupi/agent-memory/{agent-name}/
 *   - "local"   → .yupi/agent-memory-local/{agent-name}/
 *
 * The user scope previously hardcoded ~/.yupi/agent-memory/. That legacy location
...
export function resolveMemoryDir(agentName: string, scope: MemoryScope, cwd: string): string {
  ...
  switch (scope) {
    ...
    case "project":
      return join(cwd, ".yupi", "agent-memory", agentName);
    case "local":
      return join(cwd, ".yupi", "agent-memory-local", agentName);
  }
}
