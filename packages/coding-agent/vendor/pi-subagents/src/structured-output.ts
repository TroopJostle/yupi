  ...
  return defineTool({
    ...
    execute: async (_toolCallId, params) => {
      ...
      if (verdict !== true) {
        ...
        // Throwing marks the result as an error and puts the reason in front
        // of the model as a tool result, so it can correct itself inside this
        // same run. This is where most mismatches are resolved; the
        // prompt-level retry is the backstop.
        throw new Error(
