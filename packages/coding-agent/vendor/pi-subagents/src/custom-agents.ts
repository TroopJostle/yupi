/**
 * Load Yupi agent definitions from project and user configuration.
 */
 ...
 *   1. Project:   <cwd>/.yupi/agents/*.md (authoritative — also where /agents writes)
 *   2. Global:    $YUPI_CODING_AGENT_DIR/agents/*.md (default: ~/.yupi/agent/agents/*.md)
 *
 * Project-level agents override global ones with the same name. On a name clash
 * the project location is authoritative.
...
export function loadCustomAgents(cwd: string, strict = false): Map<string, AgentConfig> {
  ...
  const projectDir = join(cwd, ".yupi", "agents");
