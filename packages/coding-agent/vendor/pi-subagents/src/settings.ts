...
// - Global:  ~/.yupi/agent/subagents.json (via getAgentDir()) — manual defaults, never written here
// - Project: <cwd>/.yupi/subagents.json — written by /agents → Settings; overrides global on load
...
export interface SubagentsSettings {
   ...
   * (`<agentDir>/settings.json`) and project-local (`<cwd>/.yupi/settings.json`),
   ...
   * For a checked-in `.yupi/agents/`, failing at startup is the point — the
   ...
   * tokens are expensive; "custom" reads `.yupi/agent-tool-description.md`
...
function projectPath(cwd: string): string {
  return join(cwd, ".yupi", "subagents.json");
}
